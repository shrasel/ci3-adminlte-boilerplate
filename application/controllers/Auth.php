...
class Auth extends CI_Controller {
	...
	public function index()
	{
		...
		elseif (!$this->ion_auth->is_super_admin()) // remove this elseif if you want to enable this for non-admins
		{
	...
	public function login()
	{
		...
		else
		{
			...
			$this->_render_page('layout/login', $this->data);
		}
	}
	...
	public function activate($id, $code=false)
	{
		...
		else if ($this->ion_auth->is_super_admin())
		{
	...
	{
		if (!$this->ion_auth->logged_in() || !$this->ion_auth->is_super_admin())
		{
			// redirect them to the home page because they must be an administrator to view this
	...
	public function create_user()
    {
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data,$groups))
        {
            ...
            $this->session->set_flashdata('_success', $this->ion_auth->messages());
            redirect("auth");
        }
        else
        {
	...
	public function edit_user($id)
	{
		...
		if (!$this->ion_auth->logged_in() || (!$this->ion_auth->is_super_admin() && !($this->ion_auth->user()->row()->id == $id)))
		{
			redirect('auth');
		}
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			// check to see if we are updating the user
			   if($this->ion_auth->update($user->id, $data))
			    ...
			    {
			    	// redirect them back to the admin page if admin, or to the base url if non admin
				    $this->session->set_flashdata('_error', $this->ion_auth->errors() );
				    if ($this->ion_auth->is_super_admin())
					{
						redirect('auth');
					}
	...
	public function update_profile()
	{
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() == FALSE)
			{
				...
				$this->session->set_flashdata('_form_data', $this->input->post());
			}
		...
		// pass the user to the view
		$this->data['user'] = $this->data['active_user'] = $user;
		$this->data['groups'] = $groups;
		$this->data['currentGroups'] = $currentGroups;
